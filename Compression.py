...
def hashsupercompression(filename,mono):
    y_file, fs = librosa.load(filename,mono=mono)
    D = librosa.stft(y_file, n_fft=64, hop_length=512)
    S_db = np.transpose(np.array(librosa.amplitude_to_db(np.abs(D), ref=np.max))).tolist()
    edited = []
    average = 0
    count = 0
    for line in S_db:
        importantpoints=[line[1],line[2],line[4],line[5],line[6]]
        for data in importantpoints:
            data += 80
            if data > 0:
                average += (data)
                count += 1
    ...
    for line in S_db:
        ...
        importantpoints=[line[1],line[2],line[4],line[5],line[6]]
        for data in importantpoints:
            category=round(((data+80)*(2*average/80))/10)
            temp.append(category)
        edited.append(temp)
    return edited
...
def compressedspectrogram(y_file,filename):
    ...
    ax.get_xaxis().set_visible(False)
    ax.get_yaxis().set_visible(False)
    fig.savefig('Plots/Spectrogram_Edited/'+filename.replace(".wav","")+"_spectogram_compressed.png", bbox_inches='tight', pad_inches=0)
    ax.get_xaxis().set_visible(True)
    ax.get_yaxis().set_visible(True)
...
def supercompressedspectogram(y_file,filename):
    ...
    ax.get_xaxis().set_visible(False)
    ax.get_yaxis().set_visible(False)
    fig.savefig('Plots/Spectrogram_Edited/'+filename.replace(".wav","")+"_spectogram_supercompressed.png", bbox_inches='tight', pad_inches=0)
    ax.get_xaxis().set_visible(True)
    ax.get_yaxis().set_visible(True)
